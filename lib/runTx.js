...
module.exports = function (opts, cb) {
  ...
  if (new BN(block.header.gasLimit).ltn(tx.gasLimit)) {
  ...
  function runCall (cb) {
    ...
    if (!opts.skipBalance && new BN(fromAccount.balance).lt(tx.getUpfrontCost())) {
      ...
      return
    } else if (!opts.skipNonce && !(new BN(fromAccount.nonce).eqn(tx.nonce))) {
    ...
    fromAccount.balance = new BN(fromAccount.balance).sub(new BN(tx.gasLimit).muln(tx.gasPrice))
